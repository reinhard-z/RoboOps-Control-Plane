...
After startup, `/v1/health/ready` verifies that the configured repository can
read the current domain state. In Postgres mode it returns `503` with a
sanitized structured error when the database is unavailable or not migrated.
